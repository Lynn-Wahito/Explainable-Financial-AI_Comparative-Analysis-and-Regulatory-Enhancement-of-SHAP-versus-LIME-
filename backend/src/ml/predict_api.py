...
from .schemas import PredictIn, PredictOut, InfoOut
...
def _coerce_num(x) -> float:
    """
    Robustly coerce incoming values to float. Handles strings like
    '2.2E-1', ' [1.23] ', etc.
    """
    ...
    if isinstance(x, str):
        ...
        if s.startswith("[") and s.endswith("]"):
            s = s[1:-1].strip()
        # remove stray commas in numbers
        s = s.replace(",", "")
        try:
            return float(s)
        except Exception:
...
def predict(payload: PredictIn):
    ...
    try:
        feats = [_coerce_num(getattr(payload, k)) for k in FEATURE_ORDER]
        ...
        return {"model": type(MODEL).__name__, "predicted_label": label, "probability": round(proba, 4)}
